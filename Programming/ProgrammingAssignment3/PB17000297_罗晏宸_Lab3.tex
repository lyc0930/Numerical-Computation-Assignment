...
\begin{figure}[H]
    ...
    \subfigure[$N = 4$时均匀插值结点构造的插值函数图像]{
        \includegraphics[scale = 0.45]{Linear_N=4.pdf}
    }
    \quad
    \subfigure[$N = 4$时Chebyshev插值结点构造的插值函数图像]{
        \includegraphics[scale = 0.45]{Chebyshev_N=4.pdf}
    }

    \subfigure[$N = 8$时均匀插值结点构造的插值函数图像]{
        \includegraphics[scale = 0.45]{Linear_N=8.pdf}
    }
    \quad
    \subfigure[$N = 8$时Chebyshev插值结点构造的插值函数图像]{
        \includegraphics[scale = 0.45]{Chebyshev_N=8.pdf}
    }

    \subfigure[$N = 16$时均匀插值结点构造的插值函数图像]{
        \includegraphics[scale = 0.45]{Linear_N=16.pdf}
    }
    \quad
    \subfigure[$N = 16$时Chebyshev插值结点构造的插值函数图像]{
        \includegraphics[scale = 0.45]{Chebyshev_N=16.pdf}
    }
...
\begin{figure}[H]
    ...
    \subfigure[$f^{(5)}(x)$的函数图像]{
        \includegraphics[scale = 0.25]{5th_Derivative.pdf}
    }
    \quad
    \subfigure[$f^{(9)}(x)$的函数图像]{
        \includegraphics[scale = 0.25]{9th_Derivative.pdf}
    }

    \subfigure[$f^{(17)}(x)$的函数图像]{
        \includegraphics[scale = 0.25]{17th_Derivative.pdf}
    }
